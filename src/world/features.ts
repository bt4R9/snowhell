...
export const BIOME_CONTENT: BiomeContent[] = [
  ...
  {
    // ★ ПАРОВОЙ ГОРОД (стимпанк). Шахтёрский город террасами на склоне: цеха,
    // трубы, узкоколейка, дирижабли. Леса нет (вырублен на топливо), деревень
...
/** вес биома 3 — ПАРОВОГО ГОРОДА (стимпанк): 0 — нет, 1 — в городе */
export function cityWeight(z: number): number {
  return biomeWeightAt(z, 3);
}
/** @deprecated старое имя (полярная ночь) — оставлено для воды/озёр */
export const nightWeight = cityWeight;
 ...
 */
// снег → вулкан → паровой город (стимпанк) → высокогорье → снова снег
